...
async function titleToMatch (driver, match) {
  var title = await driver.getTitle();
  title.should.containEql(match);
  return true;
}
async function contexts (driver) {
...
async function selectWebview (driver) {
  await driver.waitUntil(
    async () => {return await contexts(driver)}, 
...
function isAndroid9 (caps) {
    return caps.platformName && caps.platformName === 'Android' &&
        caps.platformVersion && (parseFloat(caps.platformVersion) >= 9.0);
}
tests.webTest = async function (driver) {
  await driver.url("https://saucelabs.github.io/training-test-page");
  await driver.waitUntil(
    async () => { return await titleToMatch(driver, "I am a page title")},
    {timeout: 10000, interval: 1000});
};
...
tests.webTestFraud = async function (driver) {
  await driver.url("http://foo:bar@saucelabs.com/test/guinea-pig");
  await driver.waitUntil(
    async () => {return await titleToMatch(driver, "I am a page title")},
    {timeout: 7000, interval: 700});
};
...
tests.webTestHttps = async function (driver) {
  await driver.url("https://buildslave.saucelabs.com");
  await driver.waitUntil(
    async () => {return await titleToMatch(driver, "Sauce Labs")},
    {timeout: 10000, interval: 1000});
};
tests.webTestHttpsSelfSigned = async function (driver) {
  await driver.url("https://selfsigned.buildslave.saucelabs.com");
  await driver.waitUntil(
    async () => {return await titleToMatch(driver, "Sauce Labs")},
    {timeout: 10000, interval: 1000});
};
...
tests.iosHybridTest = async function (driver, caps) {
  ...
  await driver.waitUntil(
    async () => {return await titleToMatch(driver, "Google")},
    {timeout: 10000, interval: 1000});
